...
export const StationDetailsModal = ({
  ...
  const fetchChargingStops = async (
    ...
    try {
      ...
      const data = await response.json();
      let stations = data.results.map((place: any) => ({
        id: place.place_id,
        name: place.name,
        latitude: place.geometry.location.lat,
        longitude: place.geometry.location.lng,
        chargerTypes: ['CCS', 'CHAdeMO', 'Type 2'] // Mock data
      }));
      ...
      if (vehicle) {
        ...
        stations = stations.filter((station: any) =>
          station.chargerTypes.some((type: string) =>
            compatibleChargerTypes.includes(type)
          )
        );
      }

      return stations.slice(0, 1).map((station: any) => ({
  ...
  const openAppleMaps = useCallback(async () => {
    ...
    try {
      if (chargingStops.length > 0) {
        Alert.alert(
          'Charging Stops',
          `Apple Maps does not support waypoints. Please stop at: ${chargingStops
            .map((stop) => stop.name)
            .join(', ')} en route to your destination.`
        );
      }
